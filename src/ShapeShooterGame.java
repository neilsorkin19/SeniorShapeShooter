...
public class ShapeShooterGame extends JPanel implements KeyListener, MouseInputListener {
    ...
    int pelletSize = (int) (HEIGHT / 128.0);
    static int playerSize = (int) (HEIGHT / 32.0);
    ...
    static JFrame frame = new JFrame("Shape Shooter");

    public static int framesShown = 0;

    static Player p = new Player(HEIGHT, WIDTH, playerSize);
    ...
    public static Font font = new Font("Calibri", Font.BOLD, WIDTH / 40);
    public static Font ruleFont = new Font("Calibri", Font.PLAIN, WIDTH / 60);
    ...
    private void drawMenu(Graphics2D g2d) {
        ...
        drawCenterText(g2d, 0, -(HEIGHT / 5), "Shape Shooter", font);
    ...
    private void drawObjects(Graphics2D g2d) {
        ...
        if (quadrant1Cooldown / FPS <= 5) {
            ...
            g2d.setColor(new Color(160, 200, 84));
        ...
        g2d.setColor(new Color(140, 124, 200));
        g2d.setStroke(new BasicStroke((float) (HEIGHT / 128)));
        // player
        g2d.fillOval((int) p.getPosition().getX() - playerSize / 2, (int) p.getPosition().getY() - playerSize / 2,
        ...
        g2d.setStroke(new BasicStroke((float) (HEIGHT / 512)));
        g2d.draw(spawnArea);
        // spawn area
        ...
        if (frameCooldown > 0) {
            g2d.setFont(ruleFont);
            g2d.drawString((frameCooldown * 1000) / FPS + "ms", WIDTH - WIDTH / 8, HEIGHT / 16);
        }
        if (framesShown > 180) {
            // baby
    ...
    public static void run(String[] args) throws InterruptedException {
        ShapeShooterGame pelletDodger = new ShapeShooterGame();
        ...
        while (running) {
            while (lives > 0 && !showMenu) {// game loop
                ...
                if (framesShown == 180) {// spawn baby
                    b = new Shield(p.getPosition().x, p.getPosition().y);
                }
                framesShown++;
                frameCooldown--;
        ...
        if (framesShown == 180) {
            ...
            if ((currentTime - milliStart) % 1000 <= ((int) (Math.random() * level + 1) * 1000) / FPS) {
                if ((currentTime - milliStart) % 30000 <= 15000)
                    spawning = true;
                if (testing && (currentTime - milliStart) % 30000 <= 1000 / FPS) { // survived one more level
                    level++;
                    p.speedUp();
                }
            }
    ...
    private static void createObjects() {
        ...
        if (spawning) {
            for (int i = 0; i < spawnMultiplier; i++) {
                shapes.add(new Shape((int) (Math.random() * 4 + 3),
    ...
    private static void checkCollisions() {
        ...
        for (int pel = 0; pel < pellets.size(); pel++) {
            for (int g = 0; g < shapes.size(); g++) {
                if (pellets.size() > pel && shapes.get(g).containsPellet(pellets.get(pel))) {
                    score++;
                    if (framesShown > 180)
                        b.addMass();
                    pellets.remove(pel);
                    if (shapes.get(g).getLayerCount() > 3)
                        shapes.set(g, shapes.get(g).stripLayer());
                    else
                        shapes.remove(g);
                }
            }
        ...
        for (int o = 0; o < shapes.size(); o++) {
            if (framesShown > 180 && shapes.get(o).containsBaby(b)) {
                shapes.remove(o);
            }
        }
    ...
    private static void moveObjects() {
        ...
        for (int g = 0; g < shapes.size(); g++) {// move obstacles
            shapes.get(g).move();
        }
        // move pellets
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (code == KeyEvent.VK_R && lives <= 0) {// restart
            ...
            b = new Shield(p.getPosition().x, p.getPosition().x);
            frameCooldown = 0;
            lives = 3;
            milliStart = -1;
            level = 1;
